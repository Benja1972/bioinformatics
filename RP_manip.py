...
df  =  pd.read_table(path_tb+'RAG_TLX_TAP_relativeRP_mm10mm9.txt')

# -- transform
dfs = df.sort_values('lgFC_TLXvsRAG', axis=0, ascending=False)
dfs.drop_duplicates(subset='gene_name', inplace=True)

# RP Diffirential analysis
...
if DO:
    tb_in = pd.read_table(join('tracks', 'TLX3vsRAG-results_genes.txt'), index_col=0)
    ...
    tb_out.to_csv(join('tracks', 'TLX3vsRAG-results_genesNames.txt'), sep='\t')
