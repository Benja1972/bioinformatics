...
bam_fn = '/home/sergio/Research/DNA/CIML/Bioinfo-Tutorial-Tests/metaseq-tutorial/metaseq-example/data/wgEncodeHaibTfbsK562Atf3V0416101AlnRep1_chr17.bam'
...
def run_bedtools():
    ...
    bt_result = windows.coverage(bt, counts=True).sort()
...
if __name__ == "__main__":
    ...
    print 'bt = ', bt_array.size
    print 'ms = ', ms_array.size 


    #~ # Display
    #~ fig = plt.figure()
    #~ #ax = fig.add_subplot(111)
    #~ plt.plot(np.log1p(ms_array), '-b')#, alpha=0.2)
    #~ #ax.set_xlabel('log(coverageBed counts + 1)')
    #~ plt.ylabel('log(metaseq counts + 1)')
    #~ plt.title('Read counts across 10kb-windows in chr17')
    ...
    ax.set_title('Read counts across 10kb-windows in chr17')
