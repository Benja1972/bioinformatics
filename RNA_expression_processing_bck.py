...
def rna_expression(df, n_top, geneList=[],  ttl=''):
    
    # Filter genes 
    df = df[(df.padj < 0.05)].dropna()
    
    
    df['TLX3-mean'] = log2p1((df['TLX3.1_1']+df['TLX3.1_5']+df['TLX3.1_P'])/3.)
    df['RAG-mean'] = log2p1((df['R2.RAG1W.RAG1']+df['RAGS.RAGZ']+df['RAGZ'])/3.)
    
    # === Strip expression table to gene list
    df['Gene_name']=df['Gene_name'].str.upper()
    
    if not geneList:
        geneList = list(df['Gene_name'])
    
    gs = df.loc[df['Gene_name'].isin(geneList)]
    gs.set_index('Gene_name', inplace='True')
    
    gs['FCmod'] = abs(gs['log2FoldChange'])
    top = gs.sort_values('FCmod', axis=0, ascending=False)
    top = top[:n_top].sort_values('TLX3-mean', axis=0, ascending=False)

    col =['R2.RAG1W.RAG1','RAGS.RAGZ','RAGZ','TLX3.1_1','TLX3.1_5','TLX3.1_P']
    
    fig10 = plt.figure(figsize=(6, 11))
    ax10 = fig10.add_subplot(111)
    sns.heatmap(log2p1(top[col][:n_top]), 
                cmap='RdBu_r',  
                linewidths=0.004)
    ax10.set_title('UP')
    for txt in ax10.get_yticklabels():
            txt.set_rotation(0)
    for txt in ax10.get_xticklabels():
            txt.set_rotation(90)
    
    fig10.suptitle(ttl, size='x-large')
...
def rna_scatter(df, n_top, geneList=[], ttl=''):
    ...
    # == Remove duplicates
    #~ gs.drop_duplicates(subset=gs.columns[1], inplace=True)
    
    gss = gs.sort_values('log2FoldChange', 
                        axis=0, 
                        ascending=False)
    
    up = (gs.log2FoldChange > 1.).values
    dn = (gs.log2FoldChange < -1.).values
    
    
    up_l = len(gs[gs.log2FoldChange > 1.])
    dn_l = len(gs[gs.log2FoldChange < -1.])
    tt_l = len(gs)
    ...
    for i in range(n_top):
        rx = .0003*np.random.randn()
        ry = .0003*np.random.randn()
        ax.text(gss['RAG-mean'][i]*(1.+rx),
                gss['TLX3-mean'][i]*(1.+ry),
                gss['Gene_name'][i], 
                color='#da3b3a')
        ax.text(gss['RAG-mean'][-i-1]*(1.+rx),
                gss['TLX3-mean'][-i-1]*(1.+ry),
                gss['Gene_name'][-i-1], 
                color='#00748e')
...
def rna_volcano(df, n_top, geneList=[], ttl=''):
    ...
    for i in range(n_top):
        rx = .003*np.random.randn()
        #~ ry = .0000000000*np.random.randn()
        ax.text(gss['log2FoldChange'][i]*(1.+rx),
                nlog10(gss['padj'][i]+1e-300),
                gss['Gene_name'][i], 
                color='#da3b3a')
        ax.text(gss['log2FoldChange'][-i-1]*(1.+rx),
                nlog10(gss['padj'][-i-1]+1e-300),
                gss['Gene_name'][-i-1], 
                color='#00748e')
    ...
    #~ ax.axhline(2000, color='#da3b3a', linewidth=1)
    #~ ax.axvline(-2, color='#da3b3a', linewidth=1)
    #~ ax.axvline(2, color='#da3b3a', linewidth=1)
