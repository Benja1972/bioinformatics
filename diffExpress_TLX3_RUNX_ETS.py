...
with open('tracks/TLX3_peaks_RUNX_ETS_genes.gmt') as f:
...
rn.rna_scatter(tbl, list(gene_list), 5, 'TLX3 in RUNX and ETS')
rn.rna_expression(tbl, list(gene_list), 30, 'TLX3 in RUNX and ETS')
...
tlx_gen = join('tracks', 'TLX3_peaks-RUNX.bed_genesT.csv')
...
geneT = pd.read_csv(tlx_gen, header=0, index_col='Gene', names=['ID', 'Gene'])

tlx_gn = tbl.data.loc[geneT.index]
tlx_gn = tlx_gn.dropna().sort_values('log2FoldChange', axis=0, ascending=False).drop_duplicates()
tlx_gn = ResultsTable(tlx_gn)



#~ tp100 = list(tlx_gn.head(100).index)

#~ dn100 = list(tlx_gn.tail(100).index)

#~ ft = open('top100.txt', 'w')
#~ ft.write("\n".join(tp100))
#~ ft.close()

#~ ft = open('down100.txt', 'w')
#~ ft.write("\n".join(dn100))
#~ ft.close()
...
up = (tlx_gn.log2FoldChange > 1.).values
dn = (tlx_gn.log2FoldChange < -1.).values
unch =  ((tlx_gn.log2FoldChange >= -1) & (tlx_gn.log2FoldChange <= 1)).values

#~ print tbl.data.tail(40)
...
top_axes = tlx_gn.marginal.top_hists[-1]
top_axes.set_title('Differential expression, TLX3 vs RAG');

for ax in tlx_gn.marginal.top_hists:
    ax.set_ylabel('No.\ntranscripts', rotation=0, ha='right', va='center', size=8)

for ax in tlx_gn.marginal.right_hists:
